...
class Vehicle(ListBase):
    def __init__(self, *seed):
        ...
        self.totalTime = seed[0].readyTime + seed[0].serviceLen
        self.curCapacity = seed[0].demand
        #self.depot = seed[0]
    ...
    def update(self, item):
        ...
        #print("in update: traveltiem {} srvtime {}, srv {}, arrivalTime {}, readyTime {}".format(self.travelTime(item), self.totalTime, srv, arrivalTime, item.readyTime))
    ...
    def append(self, item):
        ...
        if(item.custNo is not 0 and self.__len__() != 0): #not depot and not first item
            assert self.isFeasible(item), "Item {} is not feasible on {}; full: {} time {}\n\
